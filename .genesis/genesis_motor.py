...
def _prompt_gerador(perfil, agentes, skills, referencia=""):
    ...
    return (
        ...
        "   REGRA DO TIME (é o que faz o OS ser um time, e não uma pasta de scripts soltos):\n"
        "   a skill DELEGA ao agente do time quando a etapa pede o julgamento dele. Os agentes\n"
        "   acima são subagents REAIS e invocáveis: no passo a passo, escreva explicitamente\n"
        "   'delegue ao agente `<slug>`' (usando a ferramenta Task) para a parte que é a\n"
        "   especialidade dele, em vez de reimplementar o raciocínio do especialista dentro da\n"
        "   skill. Ex: uma skill de fechamento financeiro delega a leitura do resultado ao\n"
        "   analista e a conferência de margem ao controller. Se a etapa for puramente\n"
        "   mecânica (ler arquivo, formatar saída), faça direto, sem delegar por enfeite.\n"
        "   Cada skill deve citar pelo menos um agente do time, a menos que ela seja 100%\n"
        "   mecânica: skill que nunca chama ninguém deixa o time do reveal decorativo.\n"
